...
function getAllBookings($orderIn) {
	$sql = "Booking_Id, DATE_FORMAT(DateBooked,'%d/%m/%Y') as 'DateBooked', User_Id,DATE_FORMAT(BookingDate,'%d/%m/%Y') as 'BookingDate', Room_Id ,DATE_FORMAT(TimeStarting, '%H:%i') as 'TimeStarting',DATE_FORMAT(EndTime, '%H:%i') as 'EndTime', Module_Id , Authorised, Notes FROM Booking ORDER BY DateBooked " . $orderIn;
	return executeSelect ( $sql );
}
function getNote($id) {
...
function rommAutoComplete(){
	$sql = "RoomName FROM Room";
	return executeSelect($sql);
...
function generateDayTimetable($roomId, $date) {
	$sql = "b.DateBooked,DATE_FORMAT(b.TimeStarting, '%k') as 'TimeStart', DATE_FORMAT(b.EndTime, '%k') as 'TimeEnd', m.ModuleName,r.RoomName, CONCAT(u.Title,' ', u.FirstName,' ', u.LastName) as 'Booker' FROM Booking b JOIN Module m on 
			b.Module_Id = m.Module_Id JOIN Room r ON b.Room_Id = r.Room_ID JOIN User u ON b.User_Id = u.User_Id WHERE b.DateBooked ='" . $date . "' AND b.Room_Id ='" . $roomId . "' AND b.Authorised = '1'";
	return executeSelect ( $sql );
}
function generateweeklytimetable($roomId, $date) {
	/* day of week chage */
	$sql = "b.DateBooked,DATE_FORMAT(b.TimeStarting, '%k') as 'TimeStart',
DATE_FORMAT(b.TimeStarting, '%k') as 'TimeStart', WEEKDAY(DateBooked) AS 'Day' , 
DATE_FORMAT(b.EndTime, '%k') as 'TimeEnd', m.ModuleName,r.RoomName,CONCAT(u.Title,' ', u.FirstName,' ', u.LastName) as 'Booker' FROM Booking b JOIN Module m on b.Module_Id = 
m.Module_Id JOIN Room r ON b.Room_Id = r.Room_ID JOIN User u ON b.User_Id = u.User_Id WHERE WEEK(b.DateBooked,1) = WEEK('" . $date . "',1) 
AND b.Room_Id = '" . $roomId . "' AND b.Authorised = '1'";
	return executeSelect ( $sql );
}
...
function updateUser($userid, $title, $lastName, $firstName, $email,$jobRole, $accessLevel) {
	$sql = "User SET LastName = '" . $lastName . "', FirstName = '" . $firstName . "', Title = '" . $title . "', Email = '" . $email . "', JobRole = '" . $jobRole . "', AccessLevel = '" . $accessLevel . "' WHERE User_Id =" . $userid;
	executeUpdate ( $sql );
}

function changePassword($password,$userid){
	$sql = "User SET password = '".$password."' WHERE User_Id =" . $userid;
	executeUpdate ( $sql );
...
function addBooking($userID, $roomID, $moduleID, $dateBooked, $timeStart, $timeEnd) {
	$sql = "Booking(User_Id, Room_Id, Module_Id, BookingDate, DateBooked , TimeStarting, EndTime) VALUES ('" . $userID . "', '" . $roomID . "', '" . $moduleID . "',CURDATE(), '" . $dateBooked . "', '" . $timeStart . "', '" . $timeEnd . "')";
	return executeInsert ( $sql );
}
...
function addCourse($courseID, $courseName) {
	$sql = "Course (Course_ID, CourseName) VALUES ('".$courseID."','" . $courseName . "')";
	return executeInsert ( $sql );
}
...
function processbooking($id,$descion){
	$sql="Booking SET Authorised = '".$descion."' WHERE Booking_Id=".$id;
	executeUpdate($sql);
}
?>
